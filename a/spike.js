...
const connectSound =
  'sound.beep(392,120);time.sleep_ms(120);sound.beep(494,120);time.sleep_ms(120);sound.beep(587,150);time.sleep_ms(150);sound.beep(784,200)';
const music =
...
function regen() {
  CLP_LEFT = {
    // Face U
    U: `motor.run_to_absolute_position(port.A,  motor.absolute_position(port.A)- ${
      deg + u
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    "U'": `motor.run_to_absolute_position(port.A, motor.absolute_position(port.A)+ ${
      deg + u1
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    U2: `motor.run_to_absolute_position(port.A, motor.absolute_position(port.A)+ ${
      dog + u2
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,

    // Face L
    L: `motor.run_to_absolute_position(port.C,  motor.absolute_position(port.C) - ${
      deg + l
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    "L'": `motor.run_to_absolute_position(port.C, motor.absolute_position(port.C) + ${
      deg + l1
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    L2: `motor.run_to_absolute_position(port.C, motor.absolute_position(port.C) + ${
      dog + l2
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,

    // Face F
    F: `motor.run_to_absolute_position(port.E,  motor.absolute_position(port.E)- ${
      deg + f
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    "F'": `motor.run_to_absolute_position(port.E, motor.absolute_position(port.E)+ ${
      deg + f1
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    F2: `motor.run_to_absolute_position(port.E, motor.absolute_position(port.E)+ ${
      dog + f2
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
  };
  ...
  CLP_RIGHT = {
    // Face R
    R: `motor.run_to_absolute_position(port.D,  motor.absolute_position(port.D)- ${
      deg + r
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    "R'": `motor.run_to_absolute_position(port.D, motor.absolute_position(port.D)+ ${
      deg + r1
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    R2: `motor.run_to_absolute_position(port.D, motor.absolute_position(port.D)+ ${
      dog + r2
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,

    // Face B
    B: `motor.run_to_absolute_position(port.F,  motor.absolute_position(port.F) - ${
      deg + b
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    "B'": `motor.run_to_absolute_position(port.F, motor.absolute_position(port.F) + ${
      deg + b1
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    B2: `motor.run_to_absolute_position(port.F, motor.absolute_position(port.F) + ${
      dog + b2
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,

    // Face D
    D: `motor.run_to_absolute_position(port.B,  motor.absolute_position(port.B)- ${
      deg + d
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    "D'": `motor.run_to_absolute_position(port.B, motor.absolute_position(port.B)+ ${
      deg + d1
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
    D2: `motor.run_to_absolute_position(port.B, motor.absolute_position(port.B)+ ${
      dog + d2
    }, 1110, stop=motor.SMART_BRAKE, acceleration=100000000, deceleration=100000000);\n`,
  };
}
        ...
        if (value) {
          ...
          value.split('\n').forEach((element) => {
            if (element.startsWith('Ba')) {
              ...
              const batteryPercentageValue = batteryPercentage(
                batteryValue,
                6.0,
                8.4
              );
...
async function runMovement(move, sleep = 220, noCube = false) {
  ...
  const wait =
    (move.startsWith('B') || move.startsWith('D') ? sleep + 40 : sleep) *
    (move.endsWith('2') ? 2 : 1);
...
function ganB() {
  var b = document.getElementById('cube-view').contentWindow;
  var batteryLevel = parseInt(
    b.document.getElementById('batteryLevel').value,
    10
  );
...
async function updateBatteries() {
  ...
  await Promise.all([
    sendLine(leftWriter, clearDisplay),
    sendLine(rightWriter, clearDisplay),
  ]);
  ganB();
}
...
function startTimer(startTime) {
  ...
  timerInterval = setInterval(() => {
    ...
    document.getElementById('timer').innerHTML =
      '<i class="fa-solid fa-clock"></i> : ' + elapsed + 'S';
  }, 1);
}
...
const isOpposite = (a, b) =>
  normalize(a) && normalize(b) && oppositeFace[normalize(a)] === normalize(b);
